...
public class SpellPush extends ResistedPriestSpell
{
   ...
   public SpellPush() {}
   public SpellPush(Class<? extends IPriestGroup> group, int affinity) {
      super(NAME, Attribute.Health, (byte) 1/*resistedActions*/, false/*expires*/, group, affinity);
   }
   ...
   public void applyEffects(Arena arena) {
      ...
      while (distanceKnockedBack-- > 0) {
         if (!arena.moveToFrom(getTarget(), getCaster(), getCaster().getLimbLocation(LimbType.HAND_RIGHT, arena.getCombatMap())/*retreatFromLoc*/, null/*attackFromLimb*/)) {
            ...
            Wound wound = new Wound(getEffectivePower(),
                                    Location.BODY,
                                    getName() + " spell",
                                    getEffectivePower(),//painLevel
                                    getEffectivePower()/2,//wounds
                                    0,//bleedRate
                                    0,//armPenalty
                                    0,//movePenalty
                                    0,//knockedDownDist,
