...
public class SpellPacify extends ResistedPriestSpell implements ICastInBattle
{
   ...
   public String describeSpell() {
      return "The '" + getName() + "' spell fills the subject with an overwhelming sense of peace." +
             ...
             (Configuration.useExtendedDice ? " 2 actions (instead of the usual 1 action)" : " +5") +
   ...
   public void applyEffects(Arena arena) {
      if (getTarget().isBerserking()) {
         Byte originalTeam = arena.battle.berserkingCharactersOriginalTeamID.get(getTarget().uniqueID);
         if (originalTeam != null) {
            getTarget().teamID = originalTeam;
         }
